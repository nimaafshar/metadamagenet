...
class TrainData(Dataset):
    def __init__(self, train_idxs):
        ...
        self.elastic: iaa.ElasticTransformation = iaa.ElasticTransformation(alpha=(0.25, 1.2), sigma=0.2)
    ...
    def __getitem__(self, idx):
        ...
        # TODO: check if input shape equals img.shape

        if random.random() > 0.97:
            img = shift_channels(img,
                                 random.randint(-5, 5),
                                 random.randint(-5, 5),
                                 random.randint(-5, 5))
